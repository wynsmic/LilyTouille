import { Body, Controller, Post, HttpCode, HttpStatus, Logger, UseGuards, Request } from '@nestjs/common';
...
export class InventController {
  ...
  async inventRecipe(@Request() req: AuthenticatedRequest, @Body() dto: InventRecipeDto) {
    const taskId = `invent-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;

    this.logger.log(`enqueue recipe invention: ${dto.title} for user: ${req.user?.sub}`);
